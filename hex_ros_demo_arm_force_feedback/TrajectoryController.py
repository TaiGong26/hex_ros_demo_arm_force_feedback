...
class TrajectoryPlanner(TrajectoryControllerBase):
    ...
    def __init__(self, waypoints, timestamps ):
        """
    ...
    def start_trajectory(self):
        ...
        self.__end_time = self.__timestamps[-1]
        return True
    ...
    def get_target_position(self):
        ...
        if trajectory_time >= self.__end_time:
    ...
    def get_first_point(self):
        return self.__waypoints[0].copy()
...
class Move2TargetPlanner(TrajectoryControllerBase):
    ...
    def get_target_position(self):
        ...
        if np.allclose(self.__start_position,self.__target_position):
            self.__done = True
        
        if elapsed_time >= self.__duration or self.__done:
            return self.__target_position, True  # Reached target
        ...
        target_position = self.__start_position + s * (self.__target_position - self.__start_position)
